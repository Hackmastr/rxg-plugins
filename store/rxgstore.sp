...
bool:LoadClientData( client, bool:chain=false ) {
	...
	decl String:query[1024];
	FormatEx( query, sizeof query, 
		"INSERT INTO user (user_id,server,ingame) VALUES(%d,'%s',%d) ON DUPLICATE KEY UPDATE server=VALUES(server),ingame=VALUES(ingame)",
		account,c_ip,time,c_ip,time );
	SQL_TQuery( g_db, OnClientLoggedIn, query, pack );
...
public OnClientLoggedIn( Handle:owner, Handle:hndl, const String:error[], any:data ) {
	...
	decl String:query[1024];
	FormatEx( query, sizeof query, "SELECT locked FROM user where user_id=%d", account );
	SQL_TQuery( g_db, OnClientLockChecked, query, data );
}
...
public OnClientLockChecked( Handle:owner, Handle:hndl, const String:error[], any:data ) {
	...
	decl String:query[1024];
	FormatEx( query, sizeof query,
		"SELECT count(*) as total FROM gift WHERE recipient_id=%d AND accepted=0",
		account );
	
	SQL_TQuery( g_db, OnClientGiftsLoaded, query, data );
}
...
public OnClientGiftsLoaded( Handle:owner, Handle:hndl, const String:error[], any:data ) {
	...
	SQL_TQuery( g_db, OnClientInventoryLoaded, query, data );
}
...
public ConVar_QueryClient( QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[] ) {
	...
	decl String:query[1024];
	FormatEx( query, sizeof query, 
		"INSERT INTO quick_auth (user_id, token, server, is_member) VALUES (%d, %d, '%s', %d)",
